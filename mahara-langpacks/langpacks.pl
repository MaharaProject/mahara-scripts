...
my @branches = qw(1.3_STABLE 1.4_STABLE 1.5_STABLE 1.6_STABLE 1.7_STABLE master);
...
foreach my $lang (@langkeys) {
    ...
    if ( $remote =~ m/^lp:mahara-lang/ ) {
        $repotype = 'launchpad';
        @branches = qw(1.3_STABLE 1.4_STABLE 1.5_STABLE 1.6_STABLE 1.7_STABLE master);
    }
        ...
        my $remotebranchcmd = 'git branch -r | grep -v "HEAD" | grep "origin\/\(master\|1.3_STABLE\|1.4_STABLE\|1.5_STABLE\|1.6_STABLE\|1.7_STABLE\)$"';
