...
# @UPDATE when there is a new series
my @branches = qw(17.04_STABLE 17.10_STABLE 18.04_STABLE 18.10_STABLE 19.04_STABLE master);
...
foreach my $lang (@langkeys) {
    ...
    if ( $remote =~ m/^lp:mahara-lang/ ) {
        $repotype = 'launchpad';
        # @UPDATE when there is a new series
        @branches = qw(17.04_STABLE 17.10_STABLE 18.04_STABLE 18.10_STABLE 19.04_STABLE master);
    }
        ...
        my $remotebranchcmd = 'git branch -r | grep -v "HEAD" | grep "origin\/\(master\|17.04_STABLE\|17.10_STABLE\|18.04_STABLE\|18.10_STABLE\|19.04_STABLE\)$"';
