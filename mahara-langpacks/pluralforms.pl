...
# List pulled from http://translate.sourceforge.net/wiki/l10n/pluralforms
my $plurals = {
    ach => {
        nplurals => '2',
        plural => '(n > 1)',
...
if ( defined $plurals->{$lang}->{plural} ) {
    print "$plurals->{$lang}->{plural}\n";
    exit;
}
...
if ( $shortlang ne $lang && defined $plurals->{$shortlang}->{plural} ) {
    print "$plurals->{$shortlang}->{plural}\n";
}

# List pulled from http://translate.sourceforge.net/wiki/l10n/pluralforms
# use FindBin;
# open(my $fh, '<', "$FindBin::Bin/pluralforms.txt");
# while (<$fh>) {
#     if (m/^([a-zA-Z_]+)\s.*\snplurals\s*=\s*(\d+)\s*;\s*plural\s*=\s*(.+?)[\s;]*$/) {
#         print "    $1 => {\n        nplurals => '$2',\n        plural => '$3',\n    },\n";
#         $plural = $1;
#     }
# }
